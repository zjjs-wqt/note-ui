...
export const fixTable = (vditor: IVditor, event: KeyboardEvent, range: Range) => {
    ...
    if (cellElement) {
        ...
        // -  shift + enter cell内换行 
        // - enter 切换出表格
        // - ctrl + enter 当前cell下方添加一行


        // 换行或软换行：在 cell 中添加 br
        if (!isCtrl(event) && !event.altKey && event.shiftKey && event.key === "Enter"){
        ...
        // wqt - 表格下方新添加一行，并将光标移至下一行
        // 下方新添加一行 https://github.com/Vanessa219/vditor/issues/46
        if (matchHotKey("⌘=", event) || matchHotKey("⌘Enter", event)) {
